...
def check_releases_once(tickers: list[str], target_date: str, state: dict) -> list:
    """Make a single, non-sleeping pass looking for each ticker's release.

    This replaces poll_for_releases()'s multi-hour sleep loop. Repetition
    now comes from the workflow's cron cadence -- many short runs -- rather
    than one runner sitting idle for hours. That matters for two reasons:
    a long-running job can be evicted and lose all its progress, and it
    would otherwise hold the shared repo-write concurrency group and stall
    the one-minute price/Telegram pollers behind it.

    Progress is recorded in `state`, so consecutive runs pick up where the
    last one stopped. Returns the tickers still awaiting a release.
    """
    pending = []
    for ticker in tickers:
        if state.get(f"ew_summary_sent::{ticker}::{target_date}"):
            continue
        ...
        if data:
            ...
            print(f"[{ticker}] release detected -- summary sent.")
        else:
...
def mark_poll_window_open(mode: str, target_date: str, state: dict) -> None:
    """Record when polling for this date/mode first began.

    The give-up timeout used to be measured inside one long-lived process.
    Now that polling spans many short runs we need the start time to
    persist, so it lives in state.
    """
    key = f"ew_poll_started::{mode}::{target_date}"
    if key not in state:
        state[key] = now_et().isoformat()
        print(f"Poll window opened for {mode} {target_date}.")


def maybe_give_up(pending: list, mode: str, target_date: str, state: dict) -> None:
    """Send the one-time 'still not detected' notice once this date's poll
    window has been open longer than POLL_TIMEOUT_MINUTES."""
    if not pending:
        return
    started_raw = state.get(f"ew_poll_started::{mode}::{target_date}")
    if not started_raw:
        return
    try:
        ...
        return
    if (now_et() - started) < timedelta(minutes=POLL_TIMEOUT_MINUTES):
        return
